...
class ReleaseTrustPolicyTests(unittest.TestCase):
    ...
    def test_recovery_requires_distinct_cryptographic_proof_without_private_hashes(self):
        ...
        self.assertTrue(recovery["external_offline_backup_required_before_broad_public_distribution"])
        self.assertEqual(
    ...
    def test_public_anchor_is_pinned_without_authorizing_physical_write(self):
        policy = self.load_policy()
        gates = policy["gates"]
        ...
        self.assertTrue(TRUST_PATH.exists())
        self.assertEqual(
