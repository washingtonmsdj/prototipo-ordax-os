...
class NativePreferenceTests(unittest.TestCase):
    ...
    def test_native_host_preferences_remain_loopback_and_no_cors(self):
        ...
        self.assertIn('SYNC_CHECKPOINT_PATH = "/__ordax/native/sync-checkpoint"', text)
        self.assertIn('SYNC_CHECKPOINT_FILE = "/var/lib/ordax/sync-checkpoint.json"', text)
        self.assertIn("read_sync_checkpoint", text)
        self.assertIn("write_sync_checkpoint", text)
        self.assertIn("os.fchmod(handle.fileno(), 0o600)", text)
