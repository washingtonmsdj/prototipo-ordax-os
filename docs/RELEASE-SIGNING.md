...
All three manual workflows must be dispatched against the **same exact commit**. Each
artifact carries a machine-readable `operator-receipt.json` generated by the builder with
the exact source commit plus SHA-256 and size for its exported bytes; the Local AI receipt
also binds `source-lock.json`. The local preflight re-hashes the downloaded files and
requires all three receipts to match the same `SourceCommit`, so the artifact name alone
is never treated as provenance. These GitHub Actions artifacts are temporary
operator-transfer material only: they are not the canonical public HTTPS release channel,
are retained for one day, contain no canonical private key, do not sign or activate a
release and do not authorize physical media.
...
.\Preflight-PortableV4-Canonical.ps1 \
  ...
  -SystemReceiptPath <system-operator-receipt.json> \
  -SurfaceReceiptPath <surface-operator-receipt.json> \
  -LocalAiReceiptPath <local-ai-operator-receipt.json> \
...
The preflight validates the three public artifacts, source-lock, all three builder
receipts, canonical public trust, operator tooling, exact source-commit syntax, stable
HTTPS URLs and that the configured private-key path is a non-empty regular non-reparse
file outside the public tooling directory. It recalculates SHA-256 and size for the
downloaded bytes and rejects a receipt from any other source commit. It does **not** read
the private-key contents, derive a public key, sign,
