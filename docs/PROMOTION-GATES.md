...
MINIMAL_BOOTSTRAP_MANIFEST=PASS_CANONICAL_TRUST_PINNED
...
RELEASE_TRUST_EXTERNAL_OFFLINE_BACKUP=DEFERRED_BEFORE_BROAD_PUBLIC_DISTRIBUTION
REAL_PUBLIC_TRUST_ANCHOR=PASS_CANONICAL_PINNED
...
The trust custody/recovery/rotation policy is defined in `docs/contracts/release-trust-policy.json`. The canonical public trust artifact is now pinned and the minimal bootstrap trust group is resolved; this does not authorize physical media mutation.

The signed trust-transition protocol is implemented and CI-proven as a separate forward-resilience boundary. The cryptographic recovery proof is operator-proven and the reviewed public anchor is pinned. Independent external/offline backup custody is deferred until before broad public distribution. None of these facts authorizes the first physical write; production rotation remains blocked until stateful device activation and effective-trust selection exist.
