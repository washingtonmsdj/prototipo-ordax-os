...
AI is now an **OrdaX platform foundation**, owned by the provider-neutral `ordax.ai-runtime/1` service rather than by one app or model. The optional **Assistente** app is registered with stable id `assistant`, version `0.1.0 Beta` and bundled MVP delivery as the first UI client of that runtime. Local inference is a replaceable provider below the platform boundary and receives no shell, raw-disk or implicit administrative authority. The first local provider profile targets pinned `llama.cpp` with Qwen3.5 0.8B Q4_0; engine and model remain independently replaceable. The app/runtime contract and Creator selection contract are implemented in source; the actual signed inference binary/model package and its Stable/MVP preseed are still pending, so this snapshot does **not** claim local inference is already runnable on the physical USB. The Creator feature contract defaults **IA local** to selected while allowing omission, but the currently authorized first-proof writer is intentionally unchanged; physical feature binding requires a later authorization context after the first proof.
...
FIRST_RUN_COMPLETE_LOCALES=pt-BR,en-US,es-419,fr-FR,de-DE
...
AI_NATIVE_PLATFORM_FOUNDATION=IMPLEMENTED
AI_RUNTIME_CONTRACT=ordax.ai-runtime/1
LOCAL_AI_PROVIDER_CONTRACT=ordax.local-ai/1
ASSISTANT_IS_FIRST_AI_CLIENT=YES
LOCAL_AI_ASSISTANT_SOURCE=IMPLEMENTED
LOCAL_AI_RUNTIME_CONTRACT=ordax.ai-runtime/1
LOCAL_AI_DEFAULT_PROFILE=llama.cpp/qwen3.5-0.8b-q4_0
