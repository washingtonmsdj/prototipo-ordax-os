...
def load_plan(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(artifacts, list) or len(artifacts) != 13:
        raise ProofError("portable media plan must contain exactly 13 artifacts")
...
def prove(plan_path: Path, source_values: list[str], output: Path, proof_path: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "fat32_esp": False,
        "exfat_data": False,
        "all_13_source_bindings_verified": True,
    ...
    try:
        ...
        os.sync()
        checks["all_13_artifacts_materialized_from_core_targets"] = True
