...
5. Cobertura real de idioma da Surface para os idiomas oferecidos no OOBE. — **IN_PROGRESS**: owner + shell inglês `PASS_SOURCE`; **Arquivos en-US completo em source `PASS_SOURCE`**, inclusive feedback operacional e integração Arquivos→Notas; navegação de Ajustes/Sistema também `PASS_SOURCE`; conteúdo profundo desses apps, Conta, Notas, Internet e demais áreas ainda em migração
