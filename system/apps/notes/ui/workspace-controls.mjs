...
export function mountNotesWorkspaceControls(
  ...
  const persistEditor = (noteId, payload = null) => {
    ...
    try {
      ...
      if (status) status.textContent = t("notes.save.failed");
      return false;
    }
  };
  ...
  const scheduleSave = () => {
    ...
    try {
      ...
      if (status) status.textContent = t("notes.prepare.failed");
      return false;
    }
  };
  ...
  const renderList = (view) => {
    ...
    view.querySelector(".ordax-notes-list-count").textContent = t("notes.count", {
      count: items.length,
      unit: items.length === 1 ? t("notes.count.one") : t("notes.count.many"),
    });
    ...
    for (const note of items) {
      const row = button(
        documentObject,
        "ordax-notes-row",
        t("notes.row.open", { title: note.title || t("notes.untitled.lower") }),
        "select-note",
        "",
      );
      ...
      const time = node(documentObject, "time", "ordax-notes-row-time", formatNotesRelativeTime(note.updatedAt, locale(), t));
      row.append(icon, copy, time);
      list.append(row);
    }
    if (items.length === 0) {
      list.append(node(documentObject, "p", "ordax-notes-list-empty", query ? t("notes.list.noMatch") : t("notes.list.empty")));
    }
  ...
  const renderInlineMedia = (view, note) => {
    ...
    for (const reference of references) {
      ...
      copy.append(
        node(documentObject, "strong", "", reference.title || t("notes.image.label")),
      ...
      if (activationPort && reference.path) {
        const open = button(
          ...
          t("notes.image.openLocation"),
          "open-file-reference",
          t("notes.file.openInFiles"),
        );
  ...
  const renderReferenceControls = (view, note) => {
    ...
    heading.append(
      node(
        ...
        "strong",
        "",
        pickerState.purpose === "image" ? t("notes.file.attachImage") : t("notes.file.attachFile"),
      ),
      node(documentObject, "small", "", pickerState.path),
    );
    header.append(
      heading,
      button(documentObject, "ordax-notes-file-picker-close", t("notes.file.closePicker"), "close-file-picker", "×"),
    );
    picker.append(header);

    const navigation = node(documentObject, "div", "ordax-notes-file-picker-nav");
    const up = button(documentObject, "ordax-notes-file-picker-up", t("notes.file.up"), "file-picker-up", `↑  ${t("notes.file.upShort")}`);
    ...
    if (pickerState.pending) {
      list.append(node(documentObject, "p", "ordax-notes-file-picker-message", t("notes.file.loading")));
      ...
      if (entries.length === 0) {
        list.append(node(documentObject, "p", "ordax-notes-file-picker-message", t("notes.file.emptyFolder")));
      }
      for (const entry of entries) {
        ...
        const row = button(
          ...
          entry.kind === "directory" ? t("notes.file.openFolder", { name: entry.name }) : t("notes.file.select", { name: entry.name }),
          action,
          "",
        );
        ...
        row.append(
          ...
          node(documentObject, "small", "ordax-notes-file-picker-kind", entry.kind === "directory" ? t("notes.file.folderKind") : t("notes.file.kind")),
        );
        list.append(row);
      }
    }
    ...
    const attach = button(
      ...
      pickerState.purpose === "image" ? t("notes.file.attachImageSelected") : t("notes.file.attachFileSelected"),
      "attach-file-reference",
      pickerState.purpose === "image" ? t("notes.file.attachImage") : t("notes.file.attachFile"),
    );
  ...
  const renderEditor = (view) => {
    ...
    if (intelligenceAction) {
      ...
      intelligenceAction.textContent = intelligencePending && intelligenceNoteId === note?.id
        ? t("notes.intelligence.summarizingShort")
        : t("notes.intelligence.summarizeShort");
      intelligenceAction.title = ready
        ? t("notes.intelligence.readyTitle")
        : t("notes.intelligence.unavailable");
    }
    ...
    if (!note) {
      ...
      view.querySelector(".ordax-notes-breadcrumb").textContent = t("notes.breadcrumb", { place: modeLabel() });
      view.querySelector(".ordax-notes-save-status").textContent =
        state.persistence.scope === "device"
          ? t("notes.save.devicePlain")
          : t("notes.mode.onlySession");
    ...
    const readOnly = note.deletedAt !== null;
    const projectName = project?.name ?? t("notes.home");
    view.querySelector(".ordax-notes-breadcrumb").textContent =
      t("notes.breadcrumb", { place: projectName });
    ...
    meta.textContent = readOnly
      ? t("notes.trash.readOnlyBreadcrumb", { place: projectName })
      : t("notes.note.localBreadcrumb", { place: projectName });
    ...
    saved.textContent = readOnly
      ? t("notes.trash.restoreToEdit")
      : state.persistence.ok
        ? (state.persistence.scope === "device" ? t("notes.save.device") : t("notes.save.session"))
        : t("notes.save.localFailed");
    view.querySelector(".ordax-notes-offline-status").textContent = readOnly
      ? t("notes.trash.readOnly")
      : state.persistence.scope === "device"
        ? t("notes.offline.device")
        : t("notes.mode.onlySession");
    view.querySelector(".ordax-notes-device").textContent =
      state.persistence.scope === "device"
        ? `▱  ${t("notes.device.thisDevice")}`
  ...
  const render = () => {
    ...
    if (!slot.dataset.ordaxNotesMounted || slot.dataset.ordaxNotesLocale !== activeLocale) {
      flushEditor();
      slot.replaceChildren(buildShell(documentObject, t));
      slot.dataset.ordaxNotesMounted = "true";
      slot.dataset.ordaxNotesLocale = activeLocale;
    }
    mountedSlot = slot;
  ...
  const onClick = (event) => {
    ...
    if (["view-all", "view-favorites", "view-recent", "view-trash"].includes(action)) {
      ...
      const visible = visibleNotes(state.document, mode, query, newestFirst, locale());
